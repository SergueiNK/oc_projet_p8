    ...
    def __init__(self):
        ...
        self.products_params = {
            ...
            "fields": "pnns_groups_1,product_name,generic_name_fr,"
                      "code,url,nutrition_grade_fr,image_url,"
                      "image_nutrition_url,image_front_thumb_url"
        }
    ...
    def api_get_products(self):
        ...
        try:
            request_response = requests.get(
                self.url_request, self.products_params)
        ...
        for product in self.api_get_products():
            ...
            if self.verify_product(product):
                try:
                    ...
                    try:
                        # Check if the object of category
                        # is already exist in database
                        category_data = Category.objects.get(
                            category_name=extract_product.get(
                                'pnns_groups_1'))

                    except Exception as e:
                        # If the object of the category
                        # doesn't exist, create it
                        category_data = Category.objects.create(
                            category_name=extract_product.get('pnns_groups_1'))
                    ...
                    product_data = Product.objects.create(
                        ...
                        product_description=extract_product.get(
                            'generic_name_fr'),
                        url=extract_product.get('url'),
                        nutrition_grade=extract_product.get(
                            'nutrition_grade_fr'),
                        individual_image_url=extract_product.get(
                            'image_url'),
                        list_image_url=extract_product.get(
                            'image_front_thumb_url'),
                        image_nutrition_url=extract_product.get(
                            'image_nutrition_url'),
